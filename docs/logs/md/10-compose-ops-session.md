...
- docker compose down 한 번으로 web/redis 컨테이너와 Compose가 생성한 네트워크(app_default)까지 한꺼번에 정리되었다. docker rm을 컨테이너마다 개별 실행할 필요 없이, 종료 절차 역시 하나의 명령으로 재현 가능함을 확인했다.
- down 이후 docker compose ps 결과가 빈 목록으로 나와, 관련 리소스가 완전히 정리되었음을 확인했다.

## 확인한 항목

- docker compose logs로 서비스별 로그 확인
- docker compose down으로 전체 리소스 정리
